...
class System:
    ...
    MAX_ANGLE = radians(SYSTEM.ANGLE_LIMIT_DEG)
    RAIL_WIDTH = 3

    def __init__(self, mass, damping, angle):
    ...
    def events(self, mouse_pos, mouse_pressed):
        self.hovered_left = (mouse_pos - self._left_end).length() < SETTINGS.HANDLE_SIZE * 2
        self.hovered_right = (mouse_pos - self._right_end).length() < SETTINGS.HANDLE_SIZE * 2
    ...
    def tilt(self, angle):
        self._angle = min(max(angle, -System.MAX_ANGLE), System.MAX_ANGLE)
        self._ray = Vector(cos(angle), sin(angle))
    ...
    def render(self, display):
        ...
        if self.hovered_left or self.held_left:
            ...
            left_radius = SETTINGS.HANDLE_SIZE + SETTINGS.HANDLE_HIGHLIGHT
        else:
            left_color = COLORS.HANDLE_INACTIVE
            left_radius = SETTINGS.HANDLE_SIZE

        if self.hovered_right or self.held_right:
            right_color = COLORS.HANDLE_ACTIVE
            right_radius = SETTINGS.HANDLE_SIZE + SETTINGS.HANDLE_HIGHLIGHT
        else:
            right_color = COLORS.HANDLE_INACTIVE
            right_radius = SETTINGS.HANDLE_SIZE

        pygame.draw.line(display, COLORS.RAIL, self._left_end, self._right_end, System.RAIL_WIDTH)
